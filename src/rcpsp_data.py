...
class RCPSPInstance:
    ...
    successors: list  # list[list[int]], 0-indexed
    predecessors: list = field(init=False)
    ...
    def topological_order(self) -> list:
        """Kahn's algorithm — valid serial-SGS order."""
...
def load_bks(xlsx_path="data/bks/RCPLIB (Parameters and BKS).xlsx"):
    import pandas as pd

    return pd.read_excel(xlsx_path, sheet_name=None)  # dict of DataFrames, one per dataset sheet


# ---------------- serial SGS (schedule a priority order -> makespan) ----------------


def sgs_schedule(inst: RCPSPInstance, order: list) -> np.ndarray:
    """Serial schedule-generation scheme. `order` must be precedence-feasible.
    Returns start times; makespan = start[sink] + dur[sink]."""
    start = np.zeros(inst.n_activities, dtype=np.int64)
    finish = np.zeros(inst.n_activities, dtype=np.int64)
    for i in order:
        est = max((finish[p] for p in inst.predecessors[i]), default=0)
        t = est
        while True:
            if all(
                inst.demands[i, r]
                ...
                - sum(
                    ...
                    if start[j] <= t < finish[j]
                )
                for r in range(inst.n_renewable)
            ):
                break
            t += 1
        start[i], finish[i] = t, t + inst.durations[i]
    return start
